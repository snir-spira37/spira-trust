...
DEFAULT_RESULTS = REPO_ROOT / "research" / "nesira_policy_profile" / "nesira_phase2_identity_adapter_results.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    results = run_identity_harness(repo_root)
    output = json.dumps(results, sort_keys=True, indent=2) + "\n"
    if write_results:
        DEFAULT_RESULTS.write_text(output, encoding="utf-8", newline="\n")
    print(output, end="")
    return 0 if results["verdict"] == "NESIRA_PHASE2_IDENTITY_ADAPTER_ACCEPTED" else 1
