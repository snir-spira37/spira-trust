...
TOOL_ERROR_SCHEMA = "NESIRA_PHASE2_READ_ONLY_ASSESSMENT_TOOL_ERROR_V1"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        sys.stderr.write(_tool_error(str(exc)) + "\n")
        return 2
    except Exception:
        sys.stderr.write(_tool_error("read-only assessment tool failed without exposing internal paths") + "\n")
        return 1
...
def _tool_error(message: str) -> str:
    return _canonical_json({"error": message, "schema": TOOL_ERROR_SCHEMA})
