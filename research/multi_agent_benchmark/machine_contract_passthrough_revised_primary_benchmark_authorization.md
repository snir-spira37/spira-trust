# Machine Contract Passthrough Revised Primary Benchmark Authorization

## Status

```text
MACHINE_CONTRACT_PASSTHROUGH_REVISED_PRIMARY_AUTHORIZED

PRIMARY_BENCHMARK_EXECUTION_AUTHORIZATION_ONLY
...
CLAUDE_NATIVE_180_SESSIONS_AUTHORIZED

CODEX_NATIVE_180_SESSIONS_AUTHORIZED

THREE_HUNDRED_SIXTY_SCORED_SESSIONS_MAXIMUM

SEQUENTIAL_AGENT_EXECUTION_REQUIRED

CONCURRENT_LIVE_TRACKS_FORBIDDEN

SHARED_EXPLANATION_ONLY_PROMPT_FROZEN

REVISED_RUNNER_SEMANTICS_FROZEN

ENVELOPE_SCHEMA_V1_FROZEN

ACCEPTED_VALIDATOR_FROZEN

MVP_PASSTHROUGH_IMPLEMENTATION_FROZEN

TWELVE_PRIMARY_CASES_FROZEN

THREE_ARMS_FROZEN

FIVE_REPETITIONS_FROZEN

OLD_CLAUDE_RESULTS_PRESERVED

OLD_CODEX_RESULTS_PRESERVED
...
This document authorizes the revised primary benchmark for the accepted
...
The revised readiness result accepted:
...
Historical old-policy results remain preserved:
...
No holdout, carryover, DeepSeek, release, or additional agent sessions are
authorized.

## 5. Execution Order

The two agent tracks must run sequentially.
...
The selected order must be recorded before the first scored primary session.

At no point may two live provider benchmark sessions run concurrently.

## 6. Frozen Cases, Arms, and Repetitions

The primary benchmark must use the existing frozen primary cases from:
...
Runner mechanics may add primary iteration, checkpointing, atomic persistence,
and report generation only if those changes preserve the accepted revised
readiness semantics. They must not change:
...
## 10. Arms B and C Mechanical Gates
...
## 11. Arms B and C Explanation Gates
...
## 12. Efficiency and Cost Analysis Boundaries
...
## 13. Checkpoint and Resume Rules
...
Claude Native:
180 scored sessions authorized
